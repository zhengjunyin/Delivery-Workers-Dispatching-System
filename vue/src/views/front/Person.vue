...
const save = () => {
  ...
  request.post('/user', form).then(res => {
    if (res.code === '200') {
      ...
      ElMessage.error(res.msg || tr('Save failed'))
    }
  })
}
...
<template>
  <div class="person-container">
    <el-card class="person-card">
      <h2 class="card-title">{{ $tr('Personal Information') }}</h2>
      ...
      <el-form label-width="80px">
        <div class="avatar-container">
          ...
          <div class="avatar-tip">{{ $tr('Click to upload avatar') }}</div>
        </div>
        <el-form-item :label="$tr('Username')">
          <el-input v-model="form.username" disabled autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item :label="$tr('Nickname')">
        ...
        <el-form-item>
          <el-button type="primary" @click="save">{{ $tr('Save Changes') }}</el-button>
        </el-form-item>
      </el-form>
    </el-card>
  </div>
